        ...
        try {
            if (-not ([string]::IsNullOrEmpty(($DeviceId)))) {
                if ($AlreadyAddedList.DeviceId -contains $DeviceId) {
                    Write-LogRunbook "$DeviceId was already processed." -Caller $CmdletName
                }
                else {
                    $headers = @{
        ...
        if (Test-AADGroup -GroupId $_.AzureADGroupId -GroupName $_.AzureADGroupName) {
            try {
                ...
                if ($CurrentMemberCount -eq 0) {
                    ...
                    Write-Output "Group: '$($_.AzureADGroupName)' Id: $($_.AzureADGroupId) Member count before: 0 Devices added: $($UniqueDeviceList.count) Devices removed: 0"
                }
                else {
                ...
                Write-Output "$($_.AdvancedHuntingQuery) Query results unusable. Check the log file for more details."
            }
        }
